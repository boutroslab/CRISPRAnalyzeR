...
pol <- FALSE
norm <- TRUE
Plot_sgRNA_readcount( gene, datasets, normData, rawData, bPolar = pol, bNorm = norm, bApp = FALSE )
```
...
sorted <- TRUE
Plot_sgRNA_foldchanges( gene, groups, df, bSort = sorted, bApp = FALSE )
```
...
pol <- FALSE
sorted <- TRUE
Plot_sgRNA_offtarget( gene, "offtargets", df, bSort = sorted, bPolar = pol, bApp = FALSE )
```
...
df <- rawGenes$rawGenes
sorted <- TRUE
Plot_sgRNA_zScores( gene, groups, df, bSort = sorted, bApp = FALSE )
```
...
df <- rawGenes$rawGenes
pol <- FALSE
Plot_sgRNA_scores( gene, scores, df, type = "effic", bPolar = pol, bApp = FALSE )
```
...
df <- rawGenes$rawGenes
pol <- FALSE
Plot_sgRNA_scores( gene, scores, df, type = "ecrisp", bPolar = pol, bApp = FALSE )
```
...
Table_DT(df, colNames = c("sgRNAs", "Foldchange (log2)", "Z-Score (Foldchange)","Targets", "Target Sequence"), bRownames = FALSE, class = "stripe hover", ordering = list(1, 'asc'))
```
...
df$offtargets <- round(as.numeric(df$offtargets), digits=0)
Table_DT(df, colNames = c("sgRNAs", "Targets", "Predicted Binding Sites"), bRownames = FALSE, class = "stripe hover", ordering = list(1, 'asc'))
```
